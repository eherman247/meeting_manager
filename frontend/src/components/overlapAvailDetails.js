...
const OverlapAvailDetails = ({ timeOffs, filter, uniqueNames }) => {
  ...
  const sunOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 0 && overlap.timeEnd < 1440,
      )
    : [];
  const filteredSunOverlaps = sunOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
  filteredSunOverlaps.sort((a, b) => a.timeStart - b.timeStart);
  const monOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 1440 && overlap.timeEnd < 2880,
      )
    : [];
  const monFilteredOverlaps = monOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
  monFilteredOverlaps.sort((a, b) => a.timeStart - b.timeStart);
  const tueOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 2880 && overlap.timeEnd < 4320,
      )
    : [];
  const tueFilteredOverlaps = tueOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
  tueFilteredOverlaps.sort((a, b) => a.timeStart - b.timeStart);
  const wedOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 4320 && overlap.timeEnd < 5760,
      )
    : [];
  const wedFilteredOverlaps = wedOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
  wedFilteredOverlaps.sort((a, b) => a.timeStart - b.timeStart);
  const thuOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 5760 && overlap.timeEnd < 7200,
      )
    : [];
  const thuFilteredOverlaps = thuOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
  thuFilteredOverlaps.sort((a, b) => a.timeStart - b.timeStart);
  const friOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 7200 && overlap.timeEnd < 8640,
      )
    : [];
  const friFilteredOverlaps = friOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
  friFilteredOverlaps.sort((a, b) => a.timeStart - b.timeStart);
  const satOverlaps = consolidated
    ? consolidated.filter(
        (overlap) => overlap.timeStart >= 8640 && overlap.timeEnd < 10080,
      )
    : [];
  const satFilteredOverlaps = satOverlaps.filter(
    (overlap) => overlap.timeEnd - overlap.timeStart >= filter.timeFilter,
  );
    ...
    <div className="overlap-avail-details">
      ...
      <div className="time-offs-by-day">
        ...
        <div className="time-off-day">
          ...
          {monFilteredOverlaps &&
            monFilteredOverlaps.map((overlap) => (
              <div className="time-off-details" key={overlap.timeStart * 2}>
                <p>
                  {minToTime(overlap.timeStart % 1440)} -{" "}
                  {minToTime(overlap.timeEnd % 1440)}
                </p>
              </div>
            ))}
        </div>
        <div className="time-off-day">
          ...
          {tueFilteredOverlaps &&
            tueFilteredOverlaps.map((overlap) => (
        ...
        <div className="time-off-day">
          ...
          {wedFilteredOverlaps &&
            wedFilteredOverlaps.map((overlap) => (
        ...
        <div className="time-off-day">
          ...
          {thuFilteredOverlaps &&
            thuFilteredOverlaps.map((overlap) => (
        ...
        <div className="time-off-day">
          ...
          {friFilteredOverlaps &&
            friFilteredOverlaps.map((overlap) => (
        ...
        <div className="time-off-day">
          ...
          {satFilteredOverlaps &&
            satFilteredOverlaps.map((overlap) => (
